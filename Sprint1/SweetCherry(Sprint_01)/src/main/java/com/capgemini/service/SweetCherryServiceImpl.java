...
public class SweetCherryServiceImpl implements SweetCherryService {
	...
	static final String DESCRIPTION = " method is called from SweetCherryServiceImpl Layer";
	 ...
	 * DESCRIPTION); try {
	 * 
	 * String regex = "[A-Z][a-z]*+([ '-][a-zA-Z]+)*"; if
	 * (cupcakedetails.getCupcakeName().matches(regex)) {
	 * 
	 * CupcakeDetails cupcakeDetails = cupcakeRepository.save(cupcakedetails); if
	 * (cupcakeDetails != null) return cupcakeDetails; else throw new
	 * NullPointerException("Incorrect details");
	 * 
	 * 
	 * } else throw new
	 * NoSuchElementException("Cupcake details constraints are not satisfied"); }
	 * catch (Exception e) { throw new NoSuchCupcakeExists(
		...
		try {
			if (cupcakeId > 1) {
				...
				if (cupcakeDetails == null)
					...
					throw new NoSuchCupcakeExists("cupcake Details not found");
	 ...
	 * @Override
	 * 
	 * @Transactional public void modifyCupcakeRating(int cupcakeId, int rating)
	 * throws NoSuchCupcakeExists { methodName =
	 ...
	 * 0 && rating <= 6) { cupcakeRepository.updateRating(cupcakeId, rating); } else
	 * throw new NoSuchElementException("Rating should be between 1 to 5"); } catch
	 * (NoSuchElementException e) { throw new
	 ...
	 * logger.info(methodName +
	 * " method is called from SweetCherryServiceImpl Layer");
	 * 
	 * try { if (order.getQuantity() <= 10) { //
	 * order.setCupcakeDetails(order.getCupcakeDetails()); Orders orderObject =
	 * orderRepository.save(order); if (orderObject != null) return orderObject;
	 * else throw new NoSuchElementException("Entered order values are incorrect");
	 * } else throw new NoSuchElementException("Maximum Order quantity is 10"); }
	 * catch (NoSuchElementException e) { throw new
	 ...
	 * logger.info(methodName +
	 * " method is called from SweetCherryServiceImpl Layer"); try { if
	 * (payment.getStatus().equalsIgnoreCase("successful")) { Payment paymentObject
	 * = paymentRepository.save(payment); if (paymentObject != null) return
	 * paymentObject; else throw new NoSuchElementException("Invalid details"); }
	 * else throw new
	 * NoSuchElementException("Please update the status as 'Successful'"); } catch
	 * (NoSuchElementException e) { throw new NoSuchOrderExists(
	 ...
	 * @Override
	 * 
	 * @Transactional public Payment makeOnlinePayment(Payment payment) { return
	 * cupcakeDao.payOnline(payment);
	 * 
	 * }
	 * 
	 * @Override
	 * 
	 * @Transactional public Orders makeOnlineOrder(int orderId) throws
	 * NoSuchOrderExists { try { Orders orders = cupcakeDao.placeOrder(orderId); if
	 * (orders != null) return orders; else throw new
	 ...
	 * NoSuchElementException("Entered user details are incorrect"); } catch
	 * (NoSuchElementException e) { throw new
	 ...
	 * 
	 * 
	 * @Override public Orders cancelOnlineOrder(int orderId) throws
	 * NoSuchOrderExists { try { Orders order = cupcakeDao.cancelOrder(orderId); if
	 * (order.getOrderStatus().equalsIgnoreCase("cancelled")) return order; else
	 * throw new NoSuchElementException("Failed to cancel"); } catch
	 * (NoSuchElementException e) { throw new
	 * NoSuchOrderExists("Failed to cancel the order"); } }
	 * 
	 * @Override public List<Orders> showOrderDetailsByUserId(int userId) throws
	 * NoSuchOrderExists { try { List<Orders> ordersList =
	 * cupcakeDao.readOrderDetailsByUserId(userId); if (ordersList != null) return
	 * ordersList; else throw new NoSuchElementException("Empty list"); } catch
	 * (NoSuchElementException e) { throw new
	 * NoSuchOrderExists("There is no order"); } }
	 * 
	 * @Override
	 * 
	 * @Transactional public UserDetails modifyDeliveryAddress(UserDetails address)
	 * throws NoSuchUserExists { try { UserDetails userDetails =
	 * cupcakeDao.updateDeliveryAddress(address); if (userDetails != null) return
	 * userDetails; else throw new NoSuchElementException("Invalid User details"); }
	 * catch (NoSuchElementException e) { throw new
	 * NoSuchUserExists("The user details you have entered is incorrect! Please enter valid details"
	 * ); } }
	 * 
	 * @Override
	 * 
	 * @Transactional public UserDetails addDeliveryAddress(UserDetails userDetails)
	 * { return cupcakeDao.createDeliveryAddress(userDetails); }
	 * 
	 * @Override
	 * 
	 * @Transactional public boolean deleteDeliveryAddress(int addressId) { return
	 * cupcakeDao.removeDeliveryAddress(addressId); }
	 */
