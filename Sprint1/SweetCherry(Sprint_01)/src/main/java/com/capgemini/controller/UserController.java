...
public class UserController {
	...
	public List<CupcakeDetails> viewAllCupcakeDetails() throws NoSuchCupcakeExists {
		...
		logger.info(methodName + " method is called from UserController Method");
	 ...
	 * "application/json") public void rateMyCupcake(@PathVariable("cupcakeId") int
	 * cupcakeId, @PathVariable("rating") int rating) throws NoSuchCupcakeExists {
	 * methodName = " rateMyCupcake(int cupcakeId, int rating)";
	 * logger.info(methodName + " method is called from UserController Method");
	 * cupcakeservice.modifyCupcakeRating(cupcakeId, rating); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/addToCart
	 * 
	 * @PostMapping(path = "addToCart",consumes =
	 * MediaType.APPLICATION_JSON_VALUE,produces = MediaType.APPLICATION_JSON_VALUE)
	 * public Orders addCupcakeToCart(@RequestBody Orders order) throws
	 * NoSuchOrderExists { return cupcakeservice.addCupcakeToCart(order); }
	 * 
	 * 
	 ...
	 * payment) throws NoSuchOrderExists { methodName =
	 * "buyCupcake( Payment payment)"; logger.info(methodName +
	 * " method is called from UserController Method"); return
	 ...
	 * 
	 * @PostMapping(path = "/onlinePayment", consumes =
	 * MediaType.APPLICATION_JSON_VALUE, produces = "application/json") public
	 * Payment onlinePayment(@RequestBody Payment payment) { return
	 * cupcakeservice.makeOnlinePayment(payment); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/placeOrder
	 * 
	 * @GetMapping(path = "/placeOrder/{orderId}") public Orders
	 * placeOnlineOrder(@PathVariable("orderId") int orderId) throws
	 * NoSuchOrderExists { return cupcakeservice.makeOnlineOrder(orderId); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/userDetails
	 * 
	 * @PostMapping(path = "/userDetails", consumes =
	 * MediaType.APPLICATION_JSON_VALUE, produces = "application/json") public
	 * UserDetails addUser(UserDetails userDetails) throws NoSuchOrderExists {
	 * return cupcakeservice.addUserDetails(userDetails); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/cancelOrder/1
	 * 
	 * @GetMapping(path = "/cancelOrder/{orderId}") public Orders
	 * cancelOrder(@PathVariable("orderId") int orderid) throws NoSuchOrderExists {
	 * return cupcakeservice.cancelOnlineOrder(orderid); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/viewAllOrderDetails/1
	 * 
	 * @GetMapping(path = "/viewAllOrderDetails/{userId}") public List<Orders>
	 * viewAllOrderDetailsByUserId(@PathVariable int userId) throws
	 * NoSuchOrderExists { return cupcakeservice.showOrderDetailsByUserId(userId); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/updateDeliveryAddress
	 * 
	 * @PutMapping(path = "/updateDeliveryAddress", consumes = "application/json",
	 * produces = "application/json") public UserDetails modifyAddress(@RequestBody
	 * UserDetails userDetails) throws NoSuchUserExists { return
	 * cupcakeservice.modifyDeliveryAddress(userDetails); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/addDeliveryAddress
	 * 
	 * @PostMapping(path = "/addDeliveryAddress", consumes = "application/json",
	 * produces = "application/json") public UserDetails
	 * addDeliveryAddress(@RequestBody UserDetails userDetails) { return
	 * cupcakeservice.addDeliveryAddress(userDetails); }
	 * 
	 * // http://localhost:9090/sweetcherry-api/user/deleteDeliveryAddress/3
	 * 
	 * @DeleteMapping(path = "/deleteDeliveryAddress/{addressId}") public boolean
	 * removeCustomerDeliveryAddress(@PathVariable("addressId") int addressId) {
