...
public class SweetCherryDaoImpl implements SweetCherryDao {
	...
	public Payment payOnline(Payment payment) {
	  ...
	  public Orders placeOrder(int ordereId) { 
		...
		if(order != null && order.getOrderStatus().equalsIgnoreCase("successful")) {
		...
		/*
		 * @Override public UserDetails createUserDetails(UserDetails userDetail) {
		 * entityManager.persist(userDetail); return userDetail; }
		 */
	...
	public Orders cancelOrder(int orderId) {
		...
		if(order != null && order.getOrderStatus().equalsIgnoreCase("ordered")) {
	...
	public List<Orders> readAllOrders() {
		TypedQuery<Orders> orderQuery = entityManager.createNamedQuery("showAllOrderDetails", Orders.class);
		List<Orders> list = orderQuery.getResultList();
